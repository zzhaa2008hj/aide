...
Uses independent state tracking (`.aide/fix-state.json`) and output directory (`.aide/fix/output/`). Each stage produces paired outputs: `.md` for human review and `.json` for AI consumption. Supports resume via state file detection. Invoke via `/aide-fix "<bug description>"`.
