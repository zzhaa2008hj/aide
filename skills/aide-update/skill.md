...
This is the same idempotent bootstrap script used during initial install. It syncs `.aide/config.yaml` template changes and ensures `CLAUDE.md` has the correct `extra_skill_dirs`.
...
AIDE updated to <new-commit-short>.
  ...
  CLAUDE.md          — up to date (or: updated)
```
